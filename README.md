...
以上Oracle配置同样适用于**达梦DM8**（Oracle兼容模式，建库时`COMPATIBLE_MODE=2`）。`tables_xxl_job.oracle.sql`可直接在DM8上执行，mapper复用`classpath:/mapper/oracle/*Mapper.xml`。若改用达梦JDBC驱动，将驱动jar放入plugins目录，并调整配置如下：
...
> **注意：** 脚本末尾对自增主键（`id`）插入了显式值，DM8下`GENERATED BY DEFAULT AS IDENTITY`一般允许直接执行；若报错，可在对应INSERT前后加`SET IDENTITY_INSERT 表名 ON/OFF`。

#### 5.6金仓Kingbase说明

金仓Kingbase使用**PostgreSQL兼容模式**，复用第3节pgsql配置：SQL脚本使用`tables_xxl_job.pgsql.sql`，mapper使用`classpath:/mapper/pgsql/*Mapper.xml`。若改用Kingbase JDBC驱动，将驱动jar放入plugins目录，并调整配置如下：
